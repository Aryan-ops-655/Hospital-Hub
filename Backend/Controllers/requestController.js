...
const listRequests = async (req, res) => {
    try {
        const requests = await requestModel.find({ hospitalId: req.body.hospitalId }).sort({ createdAt: -1 });
...
const createRequest = async (req, res) => {
    try {
        const { hospitalId, userName, userContact, serviceType, details, userId } = req.body;
...
const listUserRequests = async (req, res) => {
    try {
        ...
        const requests = await requestModel.find({ userId }).populate("hospitalId").sort({ createdAt: -1 });
        res.json({ success: true, data: requests });
    } catch (error) {
...
export { listRequests, updateRequestStatus, deleteRequest, createRequest, listUserRequests };
